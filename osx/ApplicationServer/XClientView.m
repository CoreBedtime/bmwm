...
- (int)getCID {
    if (self.connection == NULL || self.xWindow == 0) return 0;
    ...
    xcb_get_property_cookie_t propCookie =
        xcb_get_property(self.connection, 0, self.xWindow, propAtom, XCB_ATOM_ANY, 0, 1);
    xcb_get_property_reply_t *propReply =
        xcb_get_property_reply(self.connection, propCookie, NULL);
    if (propReply == NULL) {
...
- (NSPoint)updateX11Pointer:(NSEvent *)event {
    ...
    if (boundsWidth <= 0 || boundsHeight <= 0
        || self.sourceFrame.size.width <= 0 || self.sourceFrame.size.height <= 0) {
        return NSZeroPoint;
    }
...
- (void)mouseDown:(NSEvent *)event {
    ...
    xcb_test_fake_input(self.connection, XCB_BUTTON_PRESS, 1, XCB_CURRENT_TIME,
                        self.rootWindow, (int16_t)rootPoint.x, (int16_t)rootPoint.y, 0);
    xcb_flush(self.connection);
}

- (void)mouseUp:(NSEvent *)event {
    NSPoint rootPoint = [self updateX11Pointer:event];
    xcb_test_fake_input(self.connection, XCB_BUTTON_RELEASE, 1, XCB_CURRENT_TIME,
                        self.rootWindow, (int16_t)rootPoint.x, (int16_t)rootPoint.y, 0);
    xcb_flush(self.connection);
}
...
- (void)rightMouseDown:(NSEvent *)event {
    [self focusOwningWindow];
    NSPoint rootPoint = [self updateX11Pointer:event];
    xcb_test_fake_input(self.connection, XCB_BUTTON_PRESS, 3, XCB_CURRENT_TIME,
                        self.rootWindow, (int16_t)rootPoint.x, (int16_t)rootPoint.y, 0);
    xcb_flush(self.connection);
}
...
- (void)rightMouseUp:(NSEvent *)event {
    [self focusOwningWindow];
    NSPoint rootPoint = [self updateX11Pointer:event];
    xcb_test_fake_input(self.connection, XCB_BUTTON_RELEASE, 3, XCB_CURRENT_TIME,
                        self.rootWindow, (int16_t)rootPoint.x, (int16_t)rootPoint.y, 0);
    xcb_flush(self.connection);
}
...
- (void)scrollWheel:(NSEvent *)event {
    ...
    if (button != 0) {
        xcb_test_fake_input(self.connection, XCB_BUTTON_PRESS, button, XCB_CURRENT_TIME,
                            self.rootWindow, (int16_t)rootPoint.x, (int16_t)rootPoint.y, 0);
        xcb_test_fake_input(self.connection, XCB_BUTTON_RELEASE, button, XCB_CURRENT_TIME,
                            self.rootWindow, (int16_t)rootPoint.x, (int16_t)rootPoint.y, 0);
        xcb_flush(self.connection);
    }
}
...
- (void)flagsChanged:(NSEvent *)event {
    ...
    if (modifierMask == NSEventModifierFlagCapsLock) {
        // CapsLock toggles: only send if the state actually changed.
