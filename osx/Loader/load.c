...
static int spawn_main_userspace(void) {
    ...
    *binary_dir = '\0';
    ...
    int  helper_len = snprintf(helper_path, sizeof(helper_path), "%s/ApplicationServer", binary_path);
    ...
    if (WIFEXITED(status)) {
        ...
        printf("[loader] ApplicationServer exited with status %d\n", exit_status);
        fflush(stdout);
        return exit_status;
    }
